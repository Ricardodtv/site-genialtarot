...
| `loja.html` | **Loja Mística** — loja por categorias (Consultas/Rituais/Emergências/Amuletos + novas automáticas). Lê os produtos de `produtos.js`; cada carta tem foto (ou símbolo se faltar), descrição, preço dourado, selo POPULAR opcional e botão ✦ Adquirir ✦ (link de compra do produto, ou WhatsApp com mensagem pronta se o link estiver vazio). Tema místico, som partilhado, contacto WhatsApp |
| `produtos.js` | **A lista de produtos da loja** — o ALBERTO edita este ficheiro (instruções lá dentro, campo a campo). 21 produtos em 6 categorias (Consultas, Rituais, Emergências, Amuletos, Mapas Astrais e Cursos — Cursos aparece sempre no fim). `destaque: true` = selo POPULAR + 1.º lugar na secção; `etiqueta: "TEXTO"` = selo violeta próprio. Fotos: subir a imagem ao GitHub e escrever o nome no campo `foto` (ficheiros consulta-*.jpg, ritual-*.jpg, amuleto-*.jpg etc.) |
